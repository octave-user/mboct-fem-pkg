...
%!   mesh.materials.iso20(mesh.groups.iso20(grp_idx_v1).elements) = 1;
%!   mesh.materials.iso20(mesh.groups.iso20(grp_idx_v2).elements) = 2;
%!   mesh.materials.iso20(mesh.groups.iso20(grp_idx_v3).elements) = 3;
%!   empty_cell = cell(1, 3);
...
%!     for k=1:numel(sol.t)
%!       vr(mesh.elements.iso20(:, i), k) = 0;
%!       for j=1:3
%!         vr(mesh.elements.iso20(:, i), k) += n(mesh.elements.iso20(:, i), j) .* sol.particle_velocity.v.iso20(:, i, j, k);
%!       endfor
%!     endfor
%!   endfor
%!   for j=1:numel(sol.t)
%!     vr(mesh.groups.iso20(grp_idx_v2).nodes, j) = 0;
%!     for i=1:3
%!       vr(mesh.groups.iso20(grp_idx_v2).nodes, j) += n(mesh.groups.iso20(grp_idx_v2).nodes, i) .* sol.vel(mesh.groups.iso20(grp_idx_v2).nodes, i, j);
%!     endfor
%!   endfor
%!   node_idx = mesh.groups.iso20(grp_idx_v2).nodes;
%!   tol = 1e-5;
