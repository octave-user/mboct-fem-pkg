...
%!   load_case.locked_dof(mesh.groups.iso4(grp_id_clamp).nodes, :) = true;
%!   grp_id_p1 = find([[mesh.groups.iso4].id] == 3);
%!   grp_id_p2 = find([[mesh.groups.iso4].id] == 2);
%!   grp_id_p3 = find([[mesh.groups.iso4].id] == 4);
%!   elem_id_p1 = mesh.groups.iso4(grp_id_p1).elements;
%!   elem_id_p2 = mesh.groups.iso4(grp_id_p2).elements;
%!   elem_id_p3 = mesh.groups.iso4(grp_id_p3).elements;
%!   elno_p1 = mesh.elements.iso4(elem_id_p1, :);
%!   elno_p2 = mesh.elements.iso4(elem_id_p2, :);
%!   elno_p3 = mesh.elements.iso4(elem_id_p3, :);
%!   load_case.pressure.iso4.elements = [elno_p1; elno_p2; elno_p3];
...
%!   A1 = sum(sum(mat_ass.surface.iso4(1:rows(elno_p1), :)));
%!   A2 = sum(sum(mat_ass.surface.iso4(rows(elno_p1) + (1:rows(elno_p2)), :)));
%!   A3 = sum(sum(mat_ass.surface.iso4(rows(elno_p1) + rows(elno_p2) + (1:rows(elno_p3)), :)));
%!   A1ref = 2 * ri * pi / 4 * b;
%!   A2ref = 2 * ro * pi / 4 * b;
