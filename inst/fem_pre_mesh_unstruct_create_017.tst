## fem_pre_mesh_unstruct_create.m: 17
%!test
%! try
%! ## TEST 17
%! close all;
...
%!   end_unwind_protect
%!   opt.mesh.order = 2;
%!   opt.mesh.elem_type = {"penta18", "quad9", "tria6h"};
...
%!   load_case.locked_dof(mesh.groups.quad9(1).nodes, 1) = true;
%!   load_case.locked_dof(mesh.groups.tria6h(1).nodes, 3) = true;
%!   load_case.locked_dof(mesh.groups.tria6h(2).nodes, 2) = true;
%!   load_case.pressure.quad9.elements = mesh.elements.quad9(mesh.groups.quad9(2).elements, :);
...
%!   endif
%!   Kt = max(max(sol_stat.stress.vmis.penta18)) / tauxx_n;
