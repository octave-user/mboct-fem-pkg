...
%!   grp_id_pz = find([[mesh.groups.iso4].id] == 4);
...
%!   endfor
%!   assert_simple(max(max(max(abs(sol_stat.stress.tau.iso8(:, :, 4:6) / max(abs(p)))))) < tol);
%!   for i=1:3
%!     assert_simple(max(max(max(abs(sol_stat.strain.epsilon.iso8(:, :, i) / epsilon(i) - 1)))) < tol);
%!   endfor
%!   assert_simple(max(max(max(abs(sol_stat.strain.epsilon.iso8(:, :, 4:6) / max(abs(epsilon)))))) < tol);
%! unwind_protect_cleanup
%!   if (numel(filename))
%!     fn = dir([filename, "*"]);
%!     for i=1:numel(fn)
