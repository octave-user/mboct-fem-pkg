## fem_pre_mesh_unstruct_create.m: 16
%!test
%! try
%! ## TEST 16
%! close all;
...
%!   end_unwind_protect
%!   opt.mesh.order = 2;
%!   opt.mesh.elem_type = {"iso27", "penta18", "quad9", "tria6h"};
...
%!   load_case.locked_dof(mesh.groups.quad9(1).nodes, 3) = true;
%!   load_case.locked_dof(mesh.groups.quad9(2).nodes, 2) = true;
%!   load_case.locked_dof(mesh.groups.quad9(3).nodes, 1) = true;
%!   load_case.pressure.quad9.elements = mesh.elements.quad9(mesh.groups.quad9(4).elements, :);
%!   load_case.pressure.quad9.p = repmat(p, rows(load_case.pressure.quad9.elements), 9);
...
%!   endif
%!   Kt = max(max(sol_stat.stress.vmis.iso27)) / tauxx_n;
