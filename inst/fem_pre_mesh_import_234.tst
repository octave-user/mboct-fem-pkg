## fem_pre_mesh_import.m:234
%!test
%! ### TEST 234 - 1D dissipative wave equation with perfectly matched layers
%! close all;
%! filename = "";
%! unwind_protect
%!   filename = tempname();
...
%!   grp_idx_impedance = find([mesh.groups.iso4.id] == 3);
...
%!   mesh.elements.acoustic_impedance.iso4.nodes = mesh.elements.iso4(mesh.groups.iso4(grp_idx_impedance).elements, :);
%!   mesh.elements.acoustic_impedance.iso4.z = repmat(z * fx^2, size(mesh.elements.acoustic_impedance.iso4.nodes));
...
%!   load_case(1).particle_velocity.iso4.vn = repmat(-real(vx0 / fx^2), numel(mesh.groups.iso4(grp_idx_input).elements), 4);
%!   load_case(2).particle_velocity.iso4.vn = repmat(-imag(vx0 / fx^2), numel(mesh.groups.iso4(grp_idx_input).elements), 4);
...
%!     mesh.elements.perfectly_matched_layers.iso8.f(i, :, :) = 1 ./ (1 - 1j * sigmax / omega);
%!   endfor
%!
%!   [mat_ass.Ka_re, ...
%!    mat_ass.Ka_im, ...
