...
%!   endif
%!   [~] = unlink([filename, ".geo"]);
%!   opt.elem_type = {"tria6", "tet10"};
%!   mesh = fem_pre_mesh_reorder(fem_pre_mesh_import([filename, ".msh"], "gmsh", opt));
%!   [~] = unlink([filename, ".msh"]);
...
%!   constr_nodes = mesh.groups.tria6(grp_idx_s1).nodes;
