## fem_pre_mesh_import.m:201
%!test
%! ### TEST 201 - 1D wave equation with energy dissipation
%! close all;
%! filename = "";
%! unwind_protect
%!   filename = tempname();
...
%!   mesh.elements.acoustic_impedance.tria6h.z = repmat(z, size(mesh.elements.acoustic_impedance.tria6h.nodes));
%!   mesh.materials.acoustic_impedance.tria6h = ones(rows(mesh.elements.acoustic_impedance.tria6h.nodes), 1, "int32");
%!   v_input = repmat(vxref(0, 0), size(mesh.elements.particle_velocity.tria6h.nodes));
%!   load_case = struct("particle_velocity", cell(1, 2));
%!   load_case(1).particle_velocity.tria6h.vn = real(-v_input);
%!   load_case(2).particle_velocity.tria6h.vn = imag(-v_input);
...
%!     ++nvx(mesh.elements.penta15(:, i));
%!     for j=1:3
%!       vx(mesh.elements.penta15(:, i), :) += reshape(sol.particle_velocity.v.penta15(:, i, j, :) * R(j, 1), rows(mesh.elements.penta15), numel(sol.t));
%!     endfor
%!   endfor
%!   for i=1:columns(vx)
%!     vx(:, i) ./= nvx;
