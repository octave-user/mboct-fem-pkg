## fem_pre_mesh_import.m:200
%!test
%! try
%! ### TEST 200 - 1D wave equation with energy dissipation
%! close all;
%! filename = "";
%! unwind_protect
%!   filename = tempname();
...
%!   v_input = repmat(vxref(0, 0), size(mesh.elements.particle_velocity.tria6.nodes));
%!   load_case = struct("particle_velocity", cell(1, 2));
%!   load_case(1).particle_velocity.tria6.vn = real(-v_input);
%!   load_case(2).particle_velocity.tria6.vn = imag(-v_input);
...
%!     ++nvx(mesh.elements.tet10(:, i));
%!     for j=1:3
%!       vx(mesh.elements.tet10(:, i), :) += reshape(sol.particle_velocity.v.tet10(:, i, j, :) * R(j, 1), rows(mesh.elements.tet10), numel(sol.t));
%!     endfor
%!   endfor
%!   for i=1:columns(vx)
%!     vx(:, i) ./= nvx;
