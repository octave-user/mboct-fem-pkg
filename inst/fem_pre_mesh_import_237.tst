## fem_pre_mesh_import.m:237
%!test
%! try
%! ### TEST 237 - 1D dissipative wave equation with perfectly matched layers
%! close all;
%! filename = "";
%! unwind_protect
%!   filename = tempname();
...
%!   grp_idx_impedance = find([mesh.groups.tria6.id] == 3);
...
%!   mesh.elements.acoustic_impedance.tria6.nodes = mesh.elements.tria6(mesh.groups.tria6(grp_idx_impedance).elements, :);
%!   mesh.elements.acoustic_impedance.tria6.z = repmat(z * fx^2, size(mesh.elements.acoustic_impedance.tria6.nodes));
...
%!   load_case(1).particle_velocity.tria6.vn = repmat(-real(vx0 / fx^2), numel(mesh.groups.tria6(grp_idx_input).elements), 6);
%!   load_case(2).particle_velocity.tria6.vn = repmat(-imag(vx0 / fx^2), numel(mesh.groups.tria6(grp_idx_input).elements), 6);
...
%!     mesh.elements.perfectly_matched_layers.tet10.f(i, :, :) = 1 ./ (1 - 1j * sigmax / omega);
%!   endfor
%!
%!   [mat_ass.Ka_re, ...
%!    mat_ass.Ka_im, ...
...
%!   sol.t = Psi / omega;
%!   sol.p = real(-1j * omega * Phi(dof_map.ndof(:, 7)) * exp(1j * Psi));
%!   solC.Phi = Phi(dof_map.ndof(:, 7));
%!   solC.PhiP = 1j * omega * Phi(dof_map.ndof(:, 7));
