## fem_pre_mesh_import.m:333
%!test
%! try
%! ## TEST 333: dynamic patch test of iso27
%! do_plot = false;
%! if (do_plot)
%!   close all;
%! endif
...
%!                                     sol_eig);
%!   tolt = eps^0.6;
%!   assert_simple(max(max(max(max(abs(sol_eig.stress.tau.iso27(:, :, :, 1:6)))))) < tolt * max(max(max(max(abs(sol_eig.stress.tau.iso27(:, :, :, 7:end)))))));
%!   tolf = eps^0.4;
