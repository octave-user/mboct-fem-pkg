## fem_pre_mesh_import.m: 468
%!test
%! try
%! ## TEST 468: thermal strain penta18
%! do_plot = false;
%! if (do_plot)
%!   close all;
%! endif
...
%!   endif
%!   [~] = unlink([filename, ".geo"]);
%!   opt_mesh.elem_type = {"penta18", "tria6h", "quad9"};
...
%!     for j=1:columns(sol_stat.strain.epsilon.penta18)
%!       assert_simple(sol_stat.strain.epsilon.penta18(:, j, i), repmat(dT * mesh.material_data.gamma, rows(sol_stat.strain.epsilon.penta18), 1), tol * abs(epsilon_th));
%!       assert_simple(sol_stat.strain.epsilon.penta18(:, j, i + 3), zeros(rows(sol_stat.strain.epsilon.penta18), 1), tol * abs(epsilon_th));
%!     endfor
%!     for j=1:columns(sol_stat.strain.epsilonm.penta18)
%!       assert_simple(sol_stat.strain.epsilonm.penta18(:, j, i), repmat(dT * mesh.material_data.gamma, rows(sol_stat.strain.epsilonm.penta18), 1), tol * abs(epsilon_th));
%!       assert_simple(sol_stat.strain.epsilonm.penta18(:, j, i + 3), zeros(rows(sol_stat.strain.epsilonm.penta18), 1), tol * abs(epsilon_th));
%!     endfor
%!   endfor
%!   for i=1:3
%!     assert_simple(sol_stat2.def(:, i), mesh.nodes(:, i) * dT * mesh.material_data.gamma, tol * max(abs([a, b, c] * epsilon_th)));
%!     for j=1:columns(sol_stat2.strain.epsilon.penta18)
%!       assert_simple(sol_stat2.strain.epsilon.penta18(:, j, i), repmat(dT * mesh.material_data.gamma, rows(sol_stat2.strain.epsilon.penta18), 1), tol * abs(epsilon_th));
%!       assert_simple(sol_stat2.strain.epsilon.penta18(:, j, i + 3), zeros(rows(sol_stat2.strain.epsilon.penta18), 1), tol * abs(epsilon_th));
%!     endfor
%!     for j=1:columns(sol_stat2.strain.epsilonm.penta18)
%!       assert_simple(sol_stat2.strain.epsilonm.penta18(:, j, i), repmat(dT * mesh.material_data.gamma, rows(sol_stat2.strain.epsilonm.penta18), 1), tol * abs(epsilon_th));
%!       assert_simple(sol_stat2.strain.epsilonm.penta18(:, j, i + 3), zeros(rows(sol_stat2.strain.epsilonm.penta18), 1), tol * abs(epsilon_th));
%!     endfor
%!   endfor
%!   assert_simple(sol_stat2.def, sol_stat.def, tol * max(max(max(abs(sol_stat.def)))));
%!   assert_simple(sol_stat2.strain.epsilon.penta18, sol_stat.strain.epsilon.penta18, tol * max(max(max(abs(sol_stat.strain.epsilon.penta18)))));
%!   assert_simple(sol_stat2.strain.epsilonm.penta18, sol_stat.strain.epsilonm.penta18, tol * max(max(max(abs(sol_stat.strain.epsilonm.penta18)))));
%!   assert_simple(max(max(max(abs(sol_stat.stress.tau.penta18)))) < tol * abs(E * epsilon_th));
%!   assert_simple(max(max(max(abs(sol_stat2.stress.tau.penta18)))) < tol * abs(E * epsilon_th));
%! unwind_protect_cleanup
%!   if (numel(filename))
%!     fn = dir([filename, "*"]);
%!     for i=1:numel(fn)
