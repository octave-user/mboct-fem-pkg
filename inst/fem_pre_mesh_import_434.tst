...
%!   grp_idx_impedance = find([mesh.groups.quad8r.id] == 3);
...
%!   mesh.materials.acoustic_boundary.quad8r(mesh.groups.quad8r(grp_idx_output).elements) = 1;
...
%!   mesh.elements.acoustic_impedance.quad8r.nodes = mesh.elements.quad8r(mesh.groups.quad8r(grp_idx_impedance).elements, :);
%!   mesh.elements.acoustic_impedance.quad8r.z = repmat(z * fx^2, size(mesh.elements.acoustic_impedance.quad8r.nodes));
...
%!   load_case(1).particle_velocity.quad8r.vn = repmat(-real(vx0 / fx^2), numel(mesh.groups.quad8r(grp_idx_input).elements), 8);
%!   load_case(2).particle_velocity.quad8r.vn = repmat(-imag(vx0 / fx^2), numel(mesh.groups.quad8r(grp_idx_input).elements), 8);
...
%!     mesh.elements.perfectly_matched_layers.iso20r.f(i, :, :) = 1 ./ (1 - 1j * sigmax / omega);
%!   endfor
%!
%!   [mat_ass.Ka_re, ...
%!    mat_ass.Ka_im, ...
