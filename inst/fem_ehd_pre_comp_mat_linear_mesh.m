...
function [mesh, mat_ass_itf, dof_map_itf, cms_opt, comp_mat, bearing_surf, sol_eig, cond_info] = fem_ehd_pre_comp_mat_linear_mesh(mesh, load_case_dof, cms_opt, bearing_surf, options)
  ...
  [mat_ass_itf, comp_mat, idx_hydro] = fem_ehd_comp_mat_filter_lambda(mat_ass_itf, dof_map_itf, comp_mat, cms_opt, num_modes_cb);
...
function [mat_ass_itf, comp_mat, cond_info] = fem_ehd_pre_comp_mat_filter_eta(mat_ass_itf, dof_map_itf, comp_mat, cms_opt, cond_info, num_modes_cb)
...
function [mat_ass_itf, comp_mat, idx_hydro] = fem_ehd_comp_mat_filter_lambda(mat_ass_itf, dof_map_itf, comp_mat, cms_opt, num_modes_cb)
  [D, A] = fem_ehd_comp_mat_tot(mat_ass_itf, comp_mat);
...
function cond_info = fem_ehd_pre_comp_mat_cond(mat_ass_itf, comp_mat, idx_hydro)
  [D, A] = fem_ehd_comp_mat_tot(mat_ass_itf, comp_mat);
  D = D(:, idx_hydro);
  G = D.' * diag(A) * D;
...
function [Phi, kappa, mat_ass_eig_p, dof_map_eig_p, load_case_eig_p, bearing_surf] = fem_ehd_pre_comp_mat_eig_p(mesh, load_case_dof, bearing_surf, cms_opt, options)
  ...
  for i=1:numel(bearing_surf)
    ...
    kappa(num_modes + (1:bearing_surf(i).options.number_of_modes)) = kappa_i;
    Phi(:, num_modes + (1:bearing_surf(i).options.number_of_modes)) = Phi_i;
    num_modes += bearing_surf(i).options.number_of_modes;
  endfor
endfunction
