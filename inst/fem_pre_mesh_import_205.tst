## fem_pre_mesh_import.m:205
%!test
%! try
%! ### TEST 205 - 1D wave equation with energy dissipation
%! close all;
%! filename = "";
%! unwind_protect
%!   filename = tempname();
...
%!   p_input = repmat(pref(0, 0), numel(mesh.groups.tria6h(grp_idx_input).nodes), 1);
%!   mesh.elements.acoustic_constr = struct("C", mat2cell(ones(1, numel(p_input)), 1, ones(1, numel(p_input))), ...
%!                                          "nodes", mat2cell(mesh.groups.tria6h(grp_idx_input).nodes, 1, ones(1, numel(mesh.groups.tria6h(grp_idx_input).nodes))), ...
%!                                          "scale", repmat({1 / omega}, 1, numel(mesh.groups.tria6h(grp_idx_input).nodes)));
