...
%!   A1 = sum(sum(mat_ass.surface.quad8(1:rows(elno_p1), :)));
%!   A2 = sum(sum(mat_ass.surface.quad8(rows(elno_p1) + (1:rows(elno_p2)), :)));
%!   A3 = sum(sum(mat_ass.surface.quad8(rows(elno_p1) + rows(elno_p2) + (1:rows(elno_p3)), :)));
%!   A1ref = 2 * ri * pi / 4 * b;
%!   A2ref = 2 * ro * pi / 4 * b;
