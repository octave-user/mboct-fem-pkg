...
function fem_ehd_pre_comp_mat_plot(comp_mat, options)
  ...
  is_struct_mesh = isfield(comp_mat, "mesh") && isfield(comp_mat.mesh, "structured");
  ...
  output_files = fem_ehd_comp_mat_plot_generic(comp_mat, options);
  
  if (is_struct_mesh)
    output_files = fem_ehd_comp_mat_plot_struct_mesh(comp_mat, output_files, options);
  endif
...
function output_files = fem_ehd_comp_mat_plot_generic(comp_mat, options)
  hnd = -1;
  output_files = {};
  
  if (options.plot_const_pressure)
    ...
    zrange = 1e6 * linspace(min(min(w)), max(max(w)), 50);
    hnd = figure("visible", "off");
    colormap jet;
  ...
  if (options.plot_matrix)
    for j=1:numel(comp_mat.bearing_surf.grid_x)
      for k=1:numel(comp_mat.bearing_surf.grid_z)
        w = zeros(numel(comp_mat.bearing_surf.grid_x), numel(comp_mat.bearing_surf.grid_z));
        for l=1:rows(w)
...
function output_file = gen_pdf_file(hnd, options, output_files)
  ...
  output_file  = fullfile(out_dir, sprintf("%s_%03d%s", out_name, length(output_files) + 1, out_ext));
  ...
  do
    ...
    if (err ~= 0 || info.size == 0)
      warning("failed to create file \"%s\"", output_file);
    endif
  until (err == 0 && info.size > 0);
