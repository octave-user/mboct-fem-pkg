...
%!   grp_idx_s4 = find([mesh.groups.tria6h.id] == 7);
%!   grp_idx_s5 = find([mesh.groups.tria6h.id] == 8);
%!   grp_idx_s6 = find([mesh.groups.tria6h.id] == 9);
...
%!   mesh.materials.tet10h(mesh.groups.tet10h(grp_idx_v1).elements) = 1;
%!   mesh.materials.tet10h(mesh.groups.tet10h(grp_idx_v2).elements) = 2;
%!   mesh.materials.tet10h(mesh.groups.tet10h(grp_idx_v3).elements) = 3;
%!   empty_cell = cell(1, 3);
...
%!   mesh.elements.fluid_struct_interface.tria6h = mesh.elements.tria6h([[mesh.groups.tria6h([grp_idx_s2, grp_idx_s3])].elements], :);
%!   mesh.elements.particle_velocity.tria6h.nodes = mesh.elements.tria6h(mesh.groups.tria6h(grp_idx_s1).elements, :);
%!   mesh.materials.particle_velocity.tria6h = repmat(int32(1), rows(mesh.elements.particle_velocity.tria6h.nodes), 1);
%!   load_case = struct("particle_velocity", cell(1, 2));
%!   load_case(1).particle_velocity.tria6h.vn = repmat(-real(v0), size(mesh.elements.particle_velocity.tria6h.nodes));
%!   load_case(2).particle_velocity.tria6h.vn = repmat(-imag(v0), size(mesh.elements.particle_velocity.tria6h.nodes));
%!   mesh.elements.acoustic_impedance.tria6h.nodes = mesh.elements.tria6h(mesh.groups.tria6h(grp_idx_s4).elements, :);
%!   mesh.elements.acoustic_impedance.tria6h.z = repmat(z3, size(mesh.elements.acoustic_impedance.tria6h.nodes));
%!   mesh.materials.acoustic_impedance.tria6h = repmat(int32(3), rows(mesh.elements.acoustic_impedance.tria6h.nodes), 1);
%!   elemid = [[mesh.groups.tria6h([grp_idx_s5, grp_idx_s6, grp_idx_s7, grp_idx_s8])].elements];
%!   nodeid = mesh.elements.tria6h(elemid, :);
...
%!     for k=1:numel(sol.t)
%!       vr(mesh.elements.tet10h(:, i), k) = 0;
%!       for j=1:3
%!         vr(mesh.elements.tet10h(:, i), k) += n(mesh.elements.tet10h(:, i), j) .* sol.particle_velocity.v.tet10h(:, i, j, k);
%!       endfor
%!     endfor
%!   endfor
%!   for j=1:numel(sol.t)
%!     vr(mesh.groups.tet10h(grp_idx_v2).nodes, j) = 0;
%!     for i=1:3
%!       vr(mesh.groups.tet10h(grp_idx_v2).nodes, j) += n(mesh.groups.tet10h(grp_idx_v2).nodes, i) .* sol.vel(mesh.groups.tet10h(grp_idx_v2).nodes, i, j);
%!     endfor
%!   endfor
%!   node_idx = mesh.groups.tet10h(grp_idx_v2).nodes;
%!   tol = 1e-5;
