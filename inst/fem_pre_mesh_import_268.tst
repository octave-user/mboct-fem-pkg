...
%!   [xx, zz] = meshgrid(x, z);
%!   xtauel = mesh.nodes(:, 1)(mesh.elements.tet20);
%!   ytauel = mesh.nodes(:, 1)(mesh.elements.tet20);
%!   ztauel = mesh.nodes(:, 3)(mesh.elements.tet20);
%!   tauxxel = sol_stat.stress.taum.tet20(:, :, 1);
%!   tauxzel = sol_stat.stress.taum.tet20(:, :, 6);
...
%!   assert_simple(tauxx(:)(idx_x), tauxx_a(:)(idx_x), 0.3e-2 * max(max(max(abs(sol_stat.stress.taum.tet20)))));
%!   assert_simple(tauxz(:)(idx_x), tauxz_a(:)(idx_x), 0.3e-2 * max(max(max(abs(sol_stat.stress.taum.tet20)))));
%! unwind_protect_cleanup
%!   if (numel(filename))
%!     fn = dir([filename, "*"]);
%!     for i=1:numel(fn)
