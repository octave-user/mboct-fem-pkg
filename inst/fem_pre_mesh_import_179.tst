...
%!   mesh.materials.acoustic_boundary.iso4(mesh.groups.iso4(grp_idx_output).elements) = 2;
...
%!   mesh.elements.acoustic_impedance.iso4.z = repmat(zT, size(mesh.elements.acoustic_impedance.iso4.nodes));
%!   mesh.materials.acoustic_impedance.iso4 = repmat(int32(2), rows(mesh.elements.acoustic_impedance.iso4.nodes), 1);
...
%!     ylim([min(min(min(min(sol.particle_velocity.v.iso8)))), max(max(max(max(sol.particle_velocity.v.iso8))))]);
%!     xlabel("x [m]");
%!     ylabel("vx [m/s]");
%!     grid on;
%!     grid minor on;
...
%!   for i=1:numel(sol.t)
%!     vx(mesh.elements.iso8(:), i) = sol.particle_velocity.v.iso8(:, :, 1, i)(:);
%!   endfor
