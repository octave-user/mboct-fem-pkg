## fem_pre_mesh_import.m:41
%!test
%! try
%! ## TEST 41: thick walled cylinder with internal pressure penta18
%! do_plot = false;
%! if (do_plot)
%!   close all;
%! endif
...
%!   theta = Theta(mesh.elements.penta18);
%!   sigmar = sigmatheta = sigmaz = zeros(size(mesh.elements.penta18));
%!   idxtens = int32([1, 4, 6;
%!                    4, 2, 5;
%!                    6, 5, 3]);
%!   for i=1:rows(mesh.elements.penta18)
%!     for j=1:columns(mesh.elements.penta18)
%!       e1 = mesh.nodes(mesh.elements.penta18(i, j), 1:3).';
%!       e1(3) = 0;
%!       e3 = [0; 0; 1];
%!       e2 = cross(e3, e1);
%!       e1 /= norm(e1);
...
%!       e3 /= norm(e3);
%!       R = [e1, e2, e3];
%!       SIGMAIJ = R.' * sol_stat.stress.taum.penta18(i, j, :)(idxtens) * R;
...
%!   endfor
%!   tol = 2e-2;
%!   assert_simple(sigmar, sigmaR(mesh.elements.penta18), tol * max(abs(sigmaR)));
%!   assert_simple(sigmatheta, sigmaTheta(mesh.elements.penta18), tol * max(abs(sigmaTheta)));
