...
%!   load_case.pressure.quad8r.p = [repmat(p1, rows(elno_p1), columns(elno_p1));
...
%!   A1 = sum(sum(mat_ass.surface.quad8r(1:rows(elno_p1), :)));
%!   A2 = sum(sum(mat_ass.surface.quad8r(rows(elno_p1) + (1:rows(elno_p2)), :)));
%!   A3 = sum(sum(mat_ass.surface.quad8r(rows(elno_p1) + rows(elno_p2) + (1:rows(elno_p3)), :)));
%!   A1ref = 2 * ri * pi / 4 * b;
%!   A2ref = 2 * ro * pi / 4 * b;
