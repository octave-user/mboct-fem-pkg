## fem_pre_mesh_import.m:232
%!test
%! ## TEST 232 - 1D dissipative wave equation with perfectly matched layers
...
%!   fputs(fd, "MeshSize{PointsOf{Volume{tmp1[1]};}}=dx1;\n");
%!   fputs(fd, "MeshSize{PointsOf{Volume{tmp2[1]};}}=dx2;\n");
...
%!   grp_idx_wall = find([mesh.groups.tria6.id] == 3);
%!   grp_idx_volume1 = find([mesh.groups.tet10.id] == 3);
%!   grp_idx_volume2 = find([mesh.groups.tet10.id] == 4);
%!   elem_idx_volume2 = mesh.groups.tet10(grp_idx_volume2).elements;
%!   [x, idx] = sort(mesh.nodes(:, 1));
%!   xPML = mesh.nodes(:, 1)(mesh.elements.tet10(elem_idx_volume2, :).') - l1;
%!   mesh.nodes(:, 1:3) = mesh.nodes(:, 1:3) * R.';
%!   load_case_dof.locked_dof = false(rows(mesh.nodes), 1);
%!   load_case_dof.locked_dof(mesh.groups.tria6(grp_idx_wall).nodes) = 1;
...
%!   mesh.materials.tet10(mesh.groups.tet10(grp_idx_volume2).elements) = 1;
...
%!   mesh.materials.acoustic_boundary.tria6(mesh.groups.tria6(grp_idx_output).elements) = 1;
...
%!   mesh.elements.perfectly_matched_layers.tet10.f = zeros(3, columns(mesh.elements.tet10), rows(mesh.elements.tet10));
...
%!     mesh.elements.perfectly_matched_layers.tet10.f(i, :, :) = 1;
%!   endfor
%!   sigmax = alphaPML ./ (deltaPML - xPML);
%!   mesh.elements.perfectly_matched_layers.tet10.f(1, :, elem_idx_volume2) = 1 ./ (1 - 1j * sigmax / k);
%!   [mat_ass.Ka_re, ...
%!    mat_ass.Ka_im, ...
%!    mat_ass.Da_re, ...
%!    mat_ass.Da_im, ...
...
%!     figure("visible", "off");
%!     hold on;
%!     plot(x * unit_meter, sol.p(idx, i) * unit_pascal, "-;p;1");
%!     plot(x * unit_meter, real(pref(x, sol.t(i))) * unit_pascal, "-;pref;0");
...
%!     endfor
%!     plot(x * unit_meter, vx(idx) * unit_meter / unit_second, "-;vn;1");
%!     plot(x * unit_meter, real(vxref(x, sol.t(i))) * unit_meter / unit_second, "-;Phi;0");
