...
%!   sol.t = 0:dt:T;
%!   solve_dae = true;
%!   if (~solve_dae && isfield(dof_map, "idx_lambda"))
%!     [Tred, Kkred, Cred, Qcred] = fem_cms_constr_elim(mesh, dof_map, mat_ass);
%!   else
%!     Tred = speye(columns(mat_ass.K))(dof_map.idx_node, :);
%!     Cred = mat_ass.M;
%!     Kkred = mat_ass.K;
%!     Qcred = mat_ass.R;
%!   endif
%!   U = zeros(columns(Kkred), numel(sol.t));
%!   theta0n = zeros(dof_map.totdof, 1);
%!   theta0n(dof_map.ndof, :) = theta0;
%!   U(:, 1) = Tred.' * theta0n(dof_map.idx_node);
%!   opts.number_of_threads = int32(4);
%!   opts.solver = "pastix";
%!   A = (1 / dt) * Cred + alpha * Kkred;
%!   Afact = fem_sol_factor(A, opts);
%!   ti = [0, (2 - sqrt(eps)) * dt, 2 * dt, 3 * dt, (3 + sqrt(eps)) * dt, T];
%!   fi = [0, 0, 1, 1, 0, 0];
%!   f = interp1(ti, fi * ts / dt, sol.t, "linear");
%!   for i=2:numel(sol.t)
%!     Qci = Qcred(:, 2) * (alpha * (1 - f(i)) + (1 - alpha) * (1 - f(i - 1))) + ...
%!           Qcred(:, 1) * (alpha * f(i) + (1 - alpha) * f(i - 1));
%!     U(:, i) = Afact \ (Cred * (U(:, i - 1)) / dt - Kkred * (U(:, i - 1) * (1 - alpha)) + Qci);
%!   endfor
%!   sol.theta = Tred(dof_map.idx_node, :) * U;
%!   opt_post.elem_types = {"tet10h", "tria6h"};
%!   opt_post.skin_only = false;
%!   #opt_post.output_step_idx = 1:100:numel(sol.t);
