...
%!     fprintf(fd, "h=%g;\n", h);
%!     fprintf(fd, "dx = %g;\n", dx);
%!     fputs(fd, "Point(1) = {0, -0.5 * w, -0.5 * h, dx};\n");
%!     fputs(fd, "Point(2) = {0,  0.5 * w, -0.5 * h, dx};\n");
%!     fputs(fd, "Point(3) = {0,  0.5 * w,  0.5 * h, dx};\n");
%!     fputs(fd, "Point(4) = {0, -0.5 * w,  0.5 * h, dx};\n");
%!     fputs(fd, "Line(1) = {1,2};\n");
%!     fputs(fd, "Line(2) = {2,3};\n");
%!     fputs(fd, "Line(3) = {3,4};\n");
%!     fputs(fd, "Line(4) = {4,1};\n");
...
%!       figure("visible", "off");
%!       hold on;
%!       plot(x1 * unit_meters, p1ref(:, i) * unit_pascal, "-;p1;r");
%!       plot(x2 * unit_meters, p2ref(:, i) * unit_pascal, "-;p2;g");
...
%!     endif
%!     assert_simple(p1n, p1ref(:, i), tol * max(max(abs(p1ref))));
%!     assert_simple(p2n, p2ref(:, i), tol * max(max(abs(p2ref))));
