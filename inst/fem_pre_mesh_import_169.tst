## fem_pre_mesh_import.m:169
%!test
%! ### TEST 169 - spherical waves
%! ####################################################
%! ## Jont Allen
...
%!   grp_idx_inlet = find([mesh.groups.tria6h.id] == 2);
%!   grp_idx_outlet = find([mesh.groups.tria6h.id] == 3);
...
%!   xi = mesh.nodes(:, 1)(mesh.elements.particle_velocity.tria6h.nodes);
%!   yi = mesh.nodes(:, 2)(mesh.elements.particle_velocity.tria6h.nodes);
%!   zi = mesh.nodes(:, 3)(mesh.elements.particle_velocity.tria6h.nodes);
...
%!   xo = mesh.nodes(:, 1)(mesh.elements.acoustic_impedance.tria6h.nodes);
%!   yo = mesh.nodes(:, 2)(mesh.elements.acoustic_impedance.tria6h.nodes);
%!   zo = mesh.nodes(:, 3)(mesh.elements.acoustic_impedance.tria6h.nodes);
%!   ro = sqrt(xo.^2 + yo.^2 + zo.^2);
%!   mesh.elements.acoustic_impedance.tria6h.z = z(ro);
...
%!   load_case(1).particle_velocity.tria6h.vn = real(vn(ri, 0));
%!   load_case(2).particle_velocity.tria6h.vn = imag(vn(ri, 0));
