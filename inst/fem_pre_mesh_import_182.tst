...
%!     vx(mesh.elements.tet10h(:, i), :) = 0;
%!     for j=1:3
%!       vx(mesh.elements.tet10h(:, i), :) += reshape(sol.particle_velocity.v.tet10h(:, i, j, :) * R(j, 1), rows(mesh.elements.tet10h), numel(sol.t));
%!     endfor
%!   endfor
%!   vx = zeros(rows(mesh.nodes), numel(sol.t));
%!   nvx = zeros(rows(mesh.nodes), 1);
%!   for i=1:columns(mesh.elements.tet10h)
%!     ++nvx(mesh.elements.tet10h(:, i));
%!     for j=1:3
%!       vx(mesh.elements.tet10h(:, i), :) += reshape(sol.particle_velocity.v.tet10h(:, i, j, :) * R(j, 1), rows(mesh.elements.tet10h), numel(sol.t));
%!     endfor
%!   endfor
