...
function [mesh, mat_ass_itf, dof_map_itf, sol_eig_itf, cms_opt, sol_tau] = fem_cms_create2(mesh, load_case_dof, cms_opt)
  ...
  elem_type_lambda = {"rbe2", "rbe3", "joints"};

  for i=1:size(sol_tau.def, 3)
    stress_i = fem_ass_matrix(mesh, ...
                              dof_map_tau, ...
                              [FEM_VEC_STRESS_CAUCH], ...
                              load_case_tau, ...
                              struct("def", sol_tau.def(:, :, i)));

    load_case_tau0.tau0 = stress_i.tau;
    load_case_tau0.lambda = struct();

    for j=1:numel(elem_type_lambda)
      if (isfield(sol_tau.lambda, elem_type_lambda{j}))
        load_case_tau0.lambda = setfield(load_case_tau0.lambda, elem_type_lambda{j}, getfield(sol_tau.lambda, elem_type_lambda{j})(:, :, i));
      endif
    endfor
