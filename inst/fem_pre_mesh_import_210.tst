...
%!   grp_idx_v3 = find([mesh.groups.iso8.id] == 3);
%!   grp_idx_s1 = find([mesh.groups.iso4.id] == 4);
%!   grp_idx_s2 = find([mesh.groups.iso4.id] == 5);
%!   grp_idx_s3 = find([mesh.groups.iso4.id] == 6);
%!   grp_idx_s4 = find([mesh.groups.iso4.id] == 7);
%!   grp_idx_s5 = find([mesh.groups.iso4.id] == 8);
%!   grp_idx_s6 = find([mesh.groups.iso4.id] == 9);
%!   grp_idx_s7 = find([mesh.groups.iso4.id] == 10);
%!   grp_idx_s8 = find([mesh.groups.iso4.id] == 11);
%!   mesh.materials.iso8 = zeros(rows(mesh.elements.iso8), 1, "int32");
%!   mesh.materials.iso8(mesh.groups.iso8(grp_idx_v1).elements) = 1;
%!   mesh.materials.iso8(mesh.groups.iso8(grp_idx_v2).elements) = 2;
%!   mesh.materials.iso8(mesh.groups.iso8(grp_idx_v3).elements) = 3;
%!   empty_cell = cell(1, 3);
...
%!   mesh.elements.acoustic_boundary.iso4 = mesh.elements.iso4([[mesh.groups.iso4([grp_idx_s1, grp_idx_s4])].elements], :);
%!   mesh.materials.acoustic_boundary.iso4 = ones(rows(mesh.elements.acoustic_boundary.iso4), 1, "int32");
%!   mesh.elements.fluid_struct_interface.iso4 = mesh.elements.iso4([[mesh.groups.iso4([grp_idx_s2, grp_idx_s3])].elements], :);
%!   mesh.elements.particle_velocity.iso4.nodes = mesh.elements.iso4(mesh.groups.iso4(grp_idx_s1).elements, :);
%!   mesh.materials.particle_velocity.iso4 = repmat(int32(1), rows(mesh.elements.particle_velocity.iso4.nodes), 1);
%!   load_case = struct("particle_velocity", cell(1, 2));
%!   load_case(1).particle_velocity.iso4.vn = repmat(-real(v0), size(mesh.elements.particle_velocity.iso4.nodes));
%!   load_case(2).particle_velocity.iso4.vn = repmat(-imag(v0), size(mesh.elements.particle_velocity.iso4.nodes));
%!   mesh.elements.acoustic_impedance.iso4.nodes = mesh.elements.iso4(mesh.groups.iso4(grp_idx_s4).elements, :);
%!   mesh.elements.acoustic_impedance.iso4.z = repmat(z3, size(mesh.elements.acoustic_impedance.iso4.nodes));
%!   mesh.materials.acoustic_impedance.iso4 = repmat(int32(3), rows(mesh.elements.acoustic_impedance.iso4.nodes), 1);
%!   elemid = [[mesh.groups.iso4([grp_idx_s5,grp_idx_s6,grp_idx_s7,grp_idx_s8])].elements];
%!   nodeid = mesh.elements.iso4(elemid, :);
...
%!     for k=1:numel(sol.t)
%!       vr(mesh.elements.iso8(:, i), k) = 0;
%!       for j=1:3
%!         vr(mesh.elements.iso8(:, i), k) += n(mesh.elements.iso8(:, i), j) .* sol.particle_velocity.v.iso8(:, i, j, k);
%!       endfor
%!     endfor
%!   endfor
%!   for j=1:numel(sol.t)
%!     vr(mesh.groups.iso8(grp_idx_v2).nodes, j) = 0;
%!     for i=1:3
%!       vr(mesh.groups.iso8(grp_idx_v2).nodes, j) += n(mesh.groups.iso8(grp_idx_v2).nodes, i) .* sol.vel(mesh.groups.iso8(grp_idx_v2).nodes, i, j);
%!     endfor
%!   endfor
%!   node_idx = mesh.groups.iso8(grp_idx_v2).nodes;
...
%!   assert_simple(sum(sol.acoustic_intensity.P.iso4(1:numel(mesh.groups.iso4(grp_idx_s1).elements), :), 1), -P(r0, sol.t), tol * max(max(abs(P(r0,sol.t)))));
%!   assert_simple(sum(sol.acoustic_intensity.P.iso4(numel(mesh.groups.iso4(grp_idx_s1).elements)+1:end, :), 1), P(r3, sol.t), tol * max(max(abs(P(r0,sol.t)))));
%! unwind_protect_cleanup
%!   if (numel(filename))
%!     fn = dir([filename, "*"]);
%!     for i=1:numel(fn)
