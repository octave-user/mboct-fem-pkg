## fem_pre_mesh_import.m: 470
%!test
%! try
%! ## TEST 470: thermal strain penta18
%! do_plot = false;
%! if (do_plot)
%!   close all;
%! endif
...
%!   if (isfield(mesh.elements, "iso27"))
%!     mesh.materials.iso27 = ones(rows(mesh.elements.iso27), 1, "int32");
%!   endif
%!   if (isfield(mesh.elements, "penta18"))
...
%!   grp_idx_inside = find([mesh.groups.quad9.id] == 2);
%!   grp_idx_outside = find([mesh.groups.quad9.id] == 3);
%!   load_case.heat_source.quad9.nodes = mesh.elements.quad9(mesh.groups.quad9(grp_idx_inside).elements, :);
%!   load_case.heat_source.quad9.q = repmat(q1, numel(mesh.groups.quad9(grp_idx_inside).elements), 9);
%!   mesh.elements.convection.quad9.nodes = mesh.elements.quad9(mesh.groups.quad9(grp_idx_outside).elements, :);
%!   mesh.elements.convection.quad9.h = repmat(alpha2, numel(mesh.groups.quad9(grp_idx_outside).elements), 9);
%!   load_case.convection.quad9.theta = repmat(Theta2, numel(mesh.groups.quad9(grp_idx_outside).elements), 9);
...
%!   idx_inside = mesh.groups.quad9(grp_idx_inside).nodes;
%!   idx_outside = mesh.groups.quad9(grp_idx_outside).nodes;
