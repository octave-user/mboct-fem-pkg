## fem_pre_mesh_struct_create.m:14
%!test
%! try
%! ## TEST14
%! close all;
...
%!                                             FEM_VEC_LOAD_CONSISTENT], ...
%!                                            load_case_dyn);
%!   mat_ass_def.K += mat_ass_def.KTAU0 + 1j * mat_ass_def.K_im;
%!   [sol_eig_def] = fem_sol_modal_damped(mesh_def, dof_map, mat_ass_def, param.number_of_modes);
...
%!   endfor
%! endfunction
%! Freact = sol_stat = sol_eig = sol_eig_def = cell(1, numel(geometry));
%! for i=1:numel(geometry)
%!   [Freact{i}, sol_stat{i}, sol_eig{i}, sol_eig_def{i}] = transmissibility(geometry(i), material, param);
%! endfor
%! tol_D = 1e-5;
%! for i=1:numel(geometry)
%!   assert_simple(max(abs(sol_eig_def{i}.D / damp.D - 1)) < tol_D);
%! endfor
%! figure("visible", "off");
%! hold on;
%! for i=1:numel(geometry)
