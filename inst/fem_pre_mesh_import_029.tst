...
%!   load_case.pressure.quad8.elements = [elno_px; elno_py];
...
%!   endfor
%!   for i=1:3
%!     assert_simple(max(max(max(abs(sol_stat.stress.tau.penta15(:, :, i) / -p(i) - 1)))) < tol);
%!   endfor
%!   assert_simple(max(max(max(abs(sol_stat.stress.tau.penta15(:, :, 4:6) / max(abs(p)))))) < tol);
%!   for i=1:3
%!     assert_simple(max(max(max(abs(sol_stat.strain.epsilon.penta15(:, :, i) / epsilon(i) - 1)))) < tol);
%!   endfor
%!   assert_simple(max(max(max(abs(sol_stat.strain.epsilon.penta15(:, :, 4:6) / max(abs(epsilon)))))) < tol);
%! unwind_protect_cleanup
%!   if (numel(filename))
%!     fn = dir([filename, "*"]);
%!     for i=1:numel(fn)
