## fem_pre_mesh_import.m:230
%!test
%! ## TEST 230 - 1D dissipative wave equation with perfectly matched layers
...
%!   elem_idx_volume2 = mesh.groups.penta15(grp_idx_volume2).elements;
%!   [x, idx] = sort(mesh.nodes(:, 1));
%!   xPML = mesh.nodes(:, 1)(mesh.elements.penta15(elem_idx_volume2, :).') - l1;
%!   mesh.nodes(:, 1:3) = mesh.nodes(:, 1:3) * R.';
%!   load_case_dof.locked_dof = false(rows(mesh.nodes), 1);
%!   load_case.locked_dof(mesh.groups.tria6h(grp_idx_wall).nodes) = true;
...
%!   mesh.elements.perfectly_matched_layers.penta15.f(1, :, elem_idx_volume2) = 1 ./ (1 - 1j * sigmax / k);
%!   [mat_ass.Ka_re, ...
%!    mat_ass.Ka_im, ...
%!    mat_ass.Da_re, ...
%!    mat_ass.Da_im, ...
