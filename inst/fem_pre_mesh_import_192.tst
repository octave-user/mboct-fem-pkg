...
%!   grp_idx_v3 = find([mesh.groups.tet10.id] == 3);
%!   grp_idx_s1 = find([mesh.groups.tria6.id] == 4);
%!   grp_idx_s2 = find([mesh.groups.tria6.id] == 5);
%!   grp_idx_s3 = find([mesh.groups.tria6.id] == 6);
%!   grp_idx_s4 = find([mesh.groups.tria6.id] == 7);
%!   grp_idx_s5 = find([mesh.groups.tria6.id] == 8);
%!   grp_idx_s6 = find([mesh.groups.tria6.id] == 9);
%!   grp_idx_s7 = find([mesh.groups.tria6.id] == 10);
%!   grp_idx_s8 = find([mesh.groups.tria6.id] == 11);
%!   mesh.materials.tet10 = zeros(rows(mesh.elements.tet10), 1, "int32");
%!   mesh.materials.tet10(mesh.groups.tet10(grp_idx_v1).elements) = 1;
%!   mesh.materials.tet10(mesh.groups.tet10(grp_idx_v2).elements) = 2;
%!   mesh.materials.tet10(mesh.groups.tet10(grp_idx_v3).elements) = 3;
%!   empty_cell = cell(1, 3);
...
%!   mesh.elements.fluid_struct_interface.tria6 = mesh.elements.tria6([[mesh.groups.tria6([grp_idx_s2, grp_idx_s3])].elements], :);
%!   mesh.elements.particle_velocity.tria6.nodes = mesh.elements.tria6(mesh.groups.tria6(grp_idx_s1).elements, :);
...
%!   load_case(1).particle_velocity.tria6.vn = repmat(-real(v0), size(mesh.elements.particle_velocity.tria6.nodes));
%!   load_case(2).particle_velocity.tria6.vn = repmat(-imag(v0), size(mesh.elements.particle_velocity.tria6.nodes));
%!   mesh.elements.acoustic_impedance.tria6.nodes = mesh.elements.tria6(mesh.groups.tria6(grp_idx_s4).elements, :);
%!   mesh.elements.acoustic_impedance.tria6.z = repmat(z3, size(mesh.elements.acoustic_impedance.tria6.nodes));
%!   mesh.materials.acoustic_impedance.tria6 = repmat(int32(3), rows(mesh.elements.acoustic_impedance.tria6.nodes), 1);
%!   elemid = [[mesh.groups.tria6([grp_idx_s5, grp_idx_s6, grp_idx_s7, grp_idx_s8])].elements];
%!   nodeid = mesh.elements.tria6(elemid, :);
%!   C = zeros(numel(nodeid), 6);
%!   nodes = zeros(numel(nodeid), 1, "int32");
%!   idxn1 = int32([4,5,6,2,3,1]);
%!   idxn2 = int32([6,4,5,6,1,5]);
%!   k = int32(0);
...
%!     for k=1:numel(sol.t)
%!       vr(mesh.elements.tet10(:, i), k) = 0;
%!       for j=1:3
%!         vr(mesh.elements.tet10(:, i), k) += n(mesh.elements.tet10(:, i), j) .* sol.particle_velocity.v.tet10(:, i, j, k);
%!       endfor
%!     endfor
%!   endfor
%!   for j=1:numel(sol.t)
%!     vr(mesh.groups.tet10(grp_idx_v2).nodes, j) = 0;
%!     for i=1:3
%!       vr(mesh.groups.tet10(grp_idx_v2).nodes, j) += n(mesh.groups.tet10(grp_idx_v2).nodes, i) .* sol.vel(mesh.groups.tet10(grp_idx_v2).nodes, i, j);
%!     endfor
%!   endfor
%!   node_idx = mesh.groups.tet10(grp_idx_v2).nodes;
%!   tol = 1e-5;
